...
```{r loaddata}
f="ect24r"
load(file.path("data",paste0(f,".Rdata")))
...
```{r meltdata}
#frac=0.5 #sample only a fraction of the data

d2=d%>%
  ungroup()%>%
   mutate(presence=as.factor(case_when(
   pres & taxa=="scr" ~ "ScR Presence",
   pres & taxa=="ocr" ~ "OcR Presence",
   TRUE ~ "Background")))%>%
#  mutate(keep=rbinom(nrow(d),1,frac), #sample
#      keep=ifelse(pres==1,1,keep))%>%
  dplyr::select(-Bf,-Gf,-Rf,-r_n,-r_z,-geometry,-pres,#-keep,
                -contains("angle"),
                -contains("sign"),
                -contains("smooth"),
                -contains("dist"),
...
dl=d2%>%  gather(key=var,
                 value = val,-X,-Y,-Z,-id,-taxa,-class,
                 -classn,-rec,-presence,-contains("density"))%>%
