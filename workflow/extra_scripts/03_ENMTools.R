...
bg.sample=2000  #background sample size from each group
bg=data %>%
  group_by(group) %>%
  sample_n(bg.sample)
...
ocr_eval <- ENMevaluate(
  occ = occ_ocr,
  occ.grp = occ_ocr.grp,
  env=rdata[[model_vars_ocr]], #select whatever layers you want to include
  bg.coords = bg.coords,
  bg.grp = bg.grp,
  method='user',
...
# can convert to SDMtune format to extract response curves and variable importance.
ocr_data <- prepareSWD(species = "ocr", p = occ_ocr, a = bg.coords,
                   env = rdata[[model_vars]])

# this refits the model - need to edit to make it identical to the above (or edit to use sdmtune instead of enmeval)
model <- train(method = "Maxnet", data = ocr_data, fc = "lqph", reg = 1, iter = 500)
